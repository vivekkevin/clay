...
-----
  ...
  --model claude-haiku-4-5   (default; cheapest model, sufficient for JSON extraction)

Cost strategy
-------------
  In llm mode the script runs the heuristic extractor FIRST. The LLM is only called
  when the heuristic finds nothing — i.e. no JSON-LD and no regex street match.
  This means well-structured pages cost $0 in LLM credits; Haiku is only invoked
  for the hard cases, keeping total cost very low while preserving full accuracy.
"""
...
PAGE_EXCERPT_CHARS = 6_000   # address info is always near the top of a page
LLM_MAX_TOKENS    = 300      # JSON reply is tiny; 300 is generous
WEB_SEARCH_USES   = 2        # one real search usually suffices

# Cost strategy: run heuristic first; only call the LLM when heuristic finds nothing.
# Flip to False to always call the LLM (slower, pricier, not usually needed).
HEURISTIC_FIRST = True

DEFAULT_MODEL = "claude-haiku-4-5"   # cheapest model; fully capable for this task
...
def process_row_llm(fields, url, model):
    """
    Two-stage pipeline to minimise LLM calls (and therefore cost):

      Stage 1 — heuristic (free):
        Fetch the page once and try JSON-LD + regex extraction.
        If a street-level address is found, return immediately — no LLM needed.

      Stage 2 — LLM / Haiku (paid, only when stage 1 fails):
        Pass the cleaned page text to claude-haiku-4-5 (cheapest model).
        Haiku uses web_search as a fallback when the page itself has no address.

    In practice this means pages with good structured markup cost nothing in LLM
    credits; Haiku is only invoked for hard / broken / sparse pages.
    """
    page_text, ctype = "", ""
    skip = looks_like_skip_url(url)

    # ── Stage 1: heuristic (always free) ──────────────────────────────────────
    if HEURISTIC_FIRST and not skip:
        page_text, ctype, err = fetch(url)
        if not err:
            addr, conf = heuristic_extract(page_text, ctype)
            if addr and conf == "street":
                return {"address": addr, "status": "Verified",
                        "notes": "Heuristic extract (no LLM used); please verify."}
        # page_text already fetched — reuse it in stage 2, don't fetch again

    # ── Stage 2: LLM fallback (only reached when heuristic found nothing) ─────
    if not page_text and not skip:
    ...
    try:
        ...
        result = _partial_or_notfound(fields, f"LLM call failed ({e}).")
